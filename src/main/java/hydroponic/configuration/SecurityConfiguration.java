package hydroponic.configuration;

import hydroponic.service.CustomUserService;
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS).and().exceptionHandling()
                ...
                .authorizeRequests((request) -> request.antMatchers( "/api/v1/auth/login", "/api/v1/signupTest", "/api/v1/signup").permitAll()
                        .antMatchers(HttpMethod.OPTIONS,"/customer/**").hasRole("CUSTOMER")
                        .antMatchers(HttpMethod.OPTIONS,"/client/**").hasRole("CLIENT")
